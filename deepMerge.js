...
module.exports = function deepMergeIf(destination, source, preferences) {
  let options = _.merge(
    { addToDestination: false, addToDestinationArray: true, overwriteWithNull: false, overwriteWithUndefined: false },
    preferences
  )
  ...
  for (let i in source) {
    ...
    if (
      (_.has(destination, i) ||
        (options.addToDestinationArray && Array.isArray(destination)) ||
        (options.addToDestination && !Array.isArray(destination))) &&
